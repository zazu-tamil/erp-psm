...
class Menu_model extends CI_Model {
    ...
    public function ensure_split_reports_menu()
    {
        ...
        // 7c. Ensure customer-balance-report exists
        $cb_menu = $this->db->where('menu_slug', 'customer-balance-report')
                            ->where('status !=', 'Delete')
                            ->get('menu_info')
                            ->row_array();
        if (!$cb_menu) {
            $this->db->insert('menu_info', array(
                'parent_id'   => $tender_parent_id,
                'menu_title'  => 'Customer Balance Report',
                'menu_slug'   => 'customer-balance-report',
                'menu_icon'   => 'fa fa-balance-scale',
                'is_header'   => 0,
                'sort_order'  => 6,
                'status'      => 'Active'
            ));
            $cb_menu_id = $this->db->insert_id();
        } else {
            $cb_menu_id = (int)$cb_menu['menu_id'];
            $this->db->where('menu_id', $cb_menu_id)->update('menu_info', array(
                'parent_id'  => $tender_parent_id,
                'menu_title' => 'Customer Balance Report',
                'menu_icon'  => 'fa fa-balance-scale',
                'sort_order' => 6,
        ...
        // 7e. Ensure supplier-invoice-report exists
        $sir_menu = $this->db->where('menu_slug', 'supplier-invoice-report')
                             ->where('status !=', 'Delete')
                             ->get('menu_info')
                             ->row_array();
        if (!$sir_menu) {
            $this->db->insert('menu_info', array(
                'parent_id'   => $supplier_parent_id,
                'menu_title'  => 'Supplier Invoice Report',
                'menu_slug'   => 'supplier-invoice-report',
                'menu_icon'   => 'fa fa-files-o',
                'is_header'   => 0,
                'sort_order'  => 2,
                'status'      => 'Active'
            ));
            $sir_menu_id = $this->db->insert_id();
        } else {
            $sir_menu_id = (int)$sir_menu['menu_id'];
            $this->db->where('menu_id', $sir_menu_id)->update('menu_info', array(
                'parent_id'  => $supplier_parent_id,
                'menu_title' => 'Supplier Invoice Report',
                'menu_icon'  => 'fa fa-files-o',
                'sort_order' => 2,
