...
<head>
    <meta charset="UTF-8">
    <title>Local Purchase Bill - <?php echo htmlspecialchars($header['invoice_no'] ?? ''); ?></title>
    <style>
        body {
...
<body>
    ...
    <table class="main-table" id="quoteTable">
        ...
        <!-- Total Excl. VAT -->
        <tr class="items-table">
            <td colspan="5" class="text-right"><strong>TOTAL TAXABLE AMOUNT</strong></td>
            <td colspan="2" class="text-right" style="width: 15%;">
                <strong><?php echo number_format($header['tot_amt_wo_tax'] ?? 0, 3); ?></strong>
            </td>
        </tr>
        <tr class="items-table">
            <td colspan="5" class="text-right"><strong>TOTAL VAT AMOUNT (<?php echo htmlspecialchars($header['vat'] ?? '0'); ?>%)</strong></td>
        ...
        <tr class="items-table" style="background:#ffff; color:#000;">
            ...
            <td colspan="2" class="text-right">
                <strong><?php echo number_format($header['tot_amt_with_tax'] ?? 0, 3); ?></strong>
            </td>
        </tr>
    ...
    <?php if (!isset($_POST['export_xls'])): ?>
        <div class="button-container no-print">
            <form action="<?php echo site_url('local-purchase-bill-list/' . ($header['local_purchase_bill_id'] ?? '')) ?>"
                method="post">
                <button type="button" class="btn btn-primary"
                    onclick="window.location.href='<?= site_url('local-purchase-bill-list') ?>'">
                    ← Back To List
                </button>
