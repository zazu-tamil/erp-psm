...
<head>
    <meta charset="UTF-8">
    <title>Delivery Partner Bill - <?php echo htmlspecialchars($header['invoice_no'] ?? ''); ?></title>
    <style>
        body {
...
<body>
    ...
    <table class="main-table" id="quoteTable">
        ...
        <tr class="items-table">
            ...
            <td colspan="2" class="text-right" style="width: 15%;">
                <strong><?php echo number_format($header['dp_charges'] ?? 0, 3); ?></strong>
            </td>
        </tr>
        <tr class="items-table">
            <td colspan="5" class="text-right"><strong>VAT AMT</strong></td>
            <td colspan="2" class="text-right">
                <strong><?php echo number_format($header['dp_vat_amt'] ?? 0, 3); ?></strong>
            </td>
        </tr>
        <tr class="items-table">
            <td colspan="5" class="text-right"><strong>TOTAL AMOUNT</strong></td>
            <td colspan="2" class="text-right" style="color: #00a65a;">
                <strong><?php echo number_format((($header['dp_charges'] ?? 0) + ($header['dp_vat_amt'] ?? 0)), 3); ?></strong>
            </td>
        </tr>
        <tr class="items-table" style="background:#ffff; color:#000;">
            <td colspan="5" class="text-right"><strong>GRAND AMOUNT</strong></td>
            <td colspan="2" class="text-right" style="color: #00a65a;">
                <strong><?php echo number_format($header['g_total'] ?? 0, 3); ?></strong>
            </td>
        </tr>
    ...
    <?php if (!isset($_POST['export_xls'])): ?>
        <div class="button-container no-print">
            <form action="<?php echo site_url('delivery-partner-bill-list/' . ($header['dp_bill_id'] ?? '')) ?>"
                method="post">
                <button type="button" class="btn btn-primary"
                    onclick="window.location.href='<?= site_url('delivery-partner-bill-list') ?>'">
                    ← Back To List
                </button>
