...
class Vendor extends CI_Controller
{
    ...
    public function vendor_quotation_add()
    {
        ...
        if ($this->input->post('mode') == 'Add') {
            ...
            redirect('vendor-quotation-list');
        }
        $sql = "
            SELECT company_id, company_name 
            FROM company_info 
    ...
    public function vendor_quotation_edit($vendor_quote_id = 0)
    {
        ...
        $sql = "
            ...
            LEFT JOIN category_info ci ON vpi.category_id = ci.category_id
            LEFT JOIN item_info ii ON vpi.item_id = ii.item_id
            WHERE vpi.vendor_quote_id = ? AND vpi.status = 'Active'
            ORDER BY vpi.vendor_quote_item_id ASC
        ";
        $query = $this->db->query($sql, [$vendor_quote_id]);
        $data['items'] = $query->result_array();
        ...
        // Status Options
        $data['quotation_status_opt'] = ['' => 'Select Status', 'Completed' => 'Completed', 'Pending' => 'Pending', 'Rejected' => 'Rejected'];
    ...
    public function get_vendor_quotation_rate_enquiry()
    {
        ...
        $sql = "
            SELECT
                ...
                a.category_id,
                a.item_id,
                CONCAT_WS(' - ', ii.item_name, ii.item_code, a.item_desc) AS full_item_name,
                a.item_desc,
                ii.item_name,
                a.uom,
                a.qty,
