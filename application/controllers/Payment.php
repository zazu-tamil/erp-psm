...
class Payment extends CI_Controller
{
    ...
    public function get_data()
    {
        ...
        if ($table == 'vendor_receipt_info_invoice_list') {

            $vendor_receipt_id = $this->input->post('vendor_receipt_id');
            ...
            $sql = "
                SELECT
                    ...
                    a.remarks,
                    a.status
                FROM vendor_receipt_info AS a
                WHERE a.status = 'Active'
                AND a.vendor_receipt_id = ?
                ORDER BY a.vendor_receipt_id DESC
            ";

            $query = $this->db->query($sql, array($vendor_receipt_id));
    ...
    public function supplier_invoice_recipt()
    {
        if (!$this->session->userdata(SESS_HD . 'logged_in'))
            redirect();
        ...
        $data['js'] = 'payment/supplier-invoice-receipt.inc';
        $data['title'] = 'Supplier Receipt';
        ...
        if ($this->input->post('mode') == 'Add') {
            ...
            $this->db->insert('vendor_receipt_info', $ins);
            $vendor_receipt_id = $this->db->insert_id();
            ...
            if (!empty($selected_idxs)) {
                foreach ($selected_idxs as $idx) {
                    ...
                    $this->db->insert('vendor_receipt_bill_info', $item_data);
                }
            }
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->session->set_flashdata('success', 'Supplier Receipt saved successfully.');
            }

            redirect('supplier-invoice-receipt');
        }
        ...
        if ($this->input->post('mode') == 'Edit') {
            ...
            $this->db->where('vendor_receipt_id', $vendor_receipt_id);
            $this->db->update('vendor_receipt_info', $upd);

            $selected_idxs = $this->input->post('selected_items') ?? [];
            $vendor_receipt_bill_id = $this->input->post('vendor_receipt_bill_id') ?? [];
            $bill_id = $this->input->post('bill_id') ?? [];
            $bill_type = $this->input->post('bill_type') ?? [];
            $bill_amount = $this->input->post('bill_amount') ?? [];
            ...
            foreach ($selected_idxs as $idx) {
                ...
                if (!empty($vendor_receipt_bill_id[$idx])) {
                    // Existing row → UPDATE
                    $this->db->where('vendor_receipt_bill_id', $vendor_receipt_bill_id[$idx]);
                    $this->db->update('vendor_receipt_bill_info', $item_data);

                    $checked_existing_ids[] = $vendor_receipt_bill_id[$idx];
                } else {
                    // New row → INSERT
                    $this->db->insert('vendor_receipt_bill_info', $item_data);
                }
            ...
            // Collect ALL vendor_receipt_bill_ids sent from the form (includes unchecked rows)
            $all_existing_ids = array_filter($vendor_receipt_bill_id, function ($v) {
                return !empty($v);
            });
            ...
            if (!empty($ids_to_delete)) {
                $this->db->where_in('vendor_receipt_bill_id', array_values($ids_to_delete));
                $this->db->update('vendor_receipt_bill_info', ['status' => 'Deleted']);
            }
        ...
        if (!empty($srch_vendor_id)) {
            $where .= " AND a.customer_id = '" . $this->db->escape_str($srch_vendor_id) . "'";
        }
        ...
        $this->db->where('status != ', 'Delete');
        $this->db->from('vendor_receipt_info as a');
        $this->db->where($where);
        $data['total_records'] = $cnt = $this->db->count_all_results();

        $data['sno'] = $this->uri->segment(2, 0);

        $config['base_url'] = trim(site_url('supplier-invoice-receipt') . '/' . $this->uri->segment(2, 0));
        $config['total_rows'] = $cnt;
        $config['per_page'] = 50;
        $config['uri_segment'] = 2;
        $config['attributes'] = array('class' => 'page-link');
        ...
        $this->load->view('page/payment/supplier-invoice-receipt', $data);
    }
    ...
    public function load_receipt_vendor_bill_data_list_ajax()
    {
        ...
        $sql = "
            ...
            LEFT JOIN vendor_receipt_bill_info vrb 
    ...
    public function load_receipt_vendor_bill_edit_list_ajax()
    {
        ...
        $sql = "
             SELECT 
                ...
                CASE 
                    WHEN (f.total_amount - IFNULL(SUM(vrb_total.bill_amount), 0)) = 0 THEN 'Paid'
                    WHEN IFNULL(SUM(vrb_total.bill_amount), 0) = 0 THEN 'Unpaid'
                    ELSE 'Partial'
                END AS payment_status,

                vrb_current.vendor_receipt_bill_id 
    ...
    public function delete_record()
    {
        ...
        if ($table == 'vendor_invoice_receipt_info') {
            ...
            $this->db->where('vendor_receipt_id', $rec_id);
            $this->db->update('vendor_receipt_bill_info', array('status' => 'Delete'));
